	...
	<div class="container">
		...
		<div class="row">
			...
			<div class="col-12 col-xl-8">
				<?php foreach ($main[0]['list_episode'] as $key => $val): ?>
					<?php if ($val['episode']==$eps): ?>
						<iframe src="<?=$val['player_url']?>" frameborder="0" width="100%" height="400px" allowfullscreen="allowfullscreen"> </iframe>
					<?php endif ?>
										...
										<?php if (@(int)$main[0]['total_episode'] <= 1): ?>
											...
											<?php else: ?>
												<?php for ($i= 0; $i < (int)$main[0]['total_episode']; $i++): ?>
													<tr>
														<td><a href="<?=base_url($this->controller)?>/detail/<?=$main[0]['imdb']?>/<?=$ses?>/<?=$i+1?>"><?=@$main[0]['title']?> eps <?=$i+1?></a></td>
													</tr>
												<?php endfor ?>
											<?php endif ?>
										</tbody>
